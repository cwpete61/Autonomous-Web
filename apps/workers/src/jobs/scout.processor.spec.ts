...
describe('ScoutProcessor', () => {
    ...
    beforeEach(async () => {
        ...
        scoutAgent = module.get<ScoutAgent>(ScoutAgent);
    });
    ...
    it('should process research successfully', async () => {
        ...
        mockPrisma.lead.update.mockResolvedValue({ id: 'lead-123', business: { name: 'Test Business' } });

        const result = await processor.handleResearch(mockJob);

        expect(scoutAgent.evaluateBusiness).toHaveBeenCalledWith({ website: 'test.com', industry: 'HVAC' });
