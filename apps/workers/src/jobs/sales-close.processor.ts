...
export class SalesCloseProcessor extends BaseProcessor {
    private readonly salesAgent: SalesCloseAgent;
    ...
    async handleSalesAction(job: Job<any>) {
        ...
        this.logger.log(`Handling sales action for lead: ${leadId} | Intent: ${intent}`);

        try {
            this.reportProgress(job, 10);

            // Handle the reply or next sales action
            const salesResult = await this.salesAgent.handleReply(job.data, replyContent, { intent });
            ...
            const lead = await this.prisma.lead.update({
                where: { id: leadId },
                data: {
                    status: salesResult.status === 'failed' ? LeadStatus.CLOSED_LOST : LeadStatus.REPLIED,
                },
            ...
            this.logger.log(`Sales action processed and event emitted for lead: ${leadId}. Status: ${lead.status}`);
